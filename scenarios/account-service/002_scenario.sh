...
sleep 2
curl -X POST -d '{"username":"002_scenario","password":"password"}' -H "Content-Type: application/json" http://$account_service_host:6000/accounts/
sleep 5
curl -X GET -H "Accept: application/json" http://$account_service_host:6000/accounts/current
sleep 5
token=$(curl -X POST -H "Authorization: Basic YnJvd3Nlcjo=" -d "scope=ui&grant_type=password&username=002_scenario&password=password" http://$auth_service_host:5000/uaa/oauth/token | jq -j .access_token)
sleep 5
